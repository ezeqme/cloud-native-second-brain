---
type: kubecon-session
event: "KubeCon + CloudNativeCon China 2025 - Hong Kong, China"
event_id: kccncchn2025
year: 2025
region: "China"
city: "Hong Kong"
country: "China"
event_date: "2025"
track: "AI + ML"
themes: ["AI ML", "Storage Data", "Kubernetes Core"]
speakers: ["Wei Chen", "China Unicom Cloud Data", "Mengxuan Li", "Dynamia"]
sched_url: https://kccncchn2025.sched.com/event/1x5iF/smart-gpu-management-dynamic-pooling-sharing-and-scheduling-for-ai-workloads-in-kubernetes-wei-chen-china-unicom-cloud-data-mengxuan-li-dynamia
youtube_search_url: https://www.youtube.com/results?search_query=Smart+GPU+Management%3A+Dynamic+Pooling%2C+Sharing%2C+and+Scheduling+for+AI+Workloads+in+Kubernetes+CNCF+KubeCon+2025
slides: []
status: parsed
---

# Smart GPU Management: Dynamic Pooling, Sharing, and Scheduling for AI Workloads in Kubernetes - Wei Chen, China Unicom Cloud Data & Mengxuan Li, Dynamia

## Identificação

- Edição: KubeCon + CloudNativeCon China 2025 - Hong Kong, China
- Trilha oficial: [[AI + ML]]
- Temas: [[AI ML]], [[Storage Data]], [[Kubernetes Core]]
- País/cidade: China / Hong Kong
- Speakers: Wei Chen, China Unicom Cloud Data, Mengxuan Li, Dynamia
- Schedule: https://kccncchn2025.sched.com/event/1x5iF/smart-gpu-management-dynamic-pooling-sharing-and-scheduling-for-ai-workloads-in-kubernetes-wei-chen-china-unicom-cloud-data-mengxuan-li-dynamia
- Busca YouTube: https://www.youtube.com/results?search_query=Smart+GPU+Management%3A+Dynamic+Pooling%2C+Sharing%2C+and+Scheduling+for+AI+Workloads+in+Kubernetes+CNCF+KubeCon+2025

## Resumo

Sessão da KubeCon sobre Smart GPU Management: Dynamic Pooling, Sharing, and Scheduling for AI Workloads in Kubernetes.

## Descrição oficial

_Descrição oficial não encontrada._

## Links

- Schedule oficial: https://kccncchn2025.sched.com/event/1x5iF/smart-gpu-management-dynamic-pooling-sharing-and-scheduling-for-ai-workloads-in-kubernetes-wei-chen-china-unicom-cloud-data-mengxuan-li-dynamia
- YouTube search: https://www.youtube.com/results?search_query=Smart+GPU+Management%3A+Dynamic+Pooling%2C+Sharing%2C+and+Scheduling+for+AI+Workloads+in+Kubernetes+CNCF+KubeCon+2025
## YouTube enrichment

- YouTube: https://www.youtube.com/watch?v=VQJBxquobQc
- YouTube title: Smart GPU Management: Dynamic Pooling, Sharing, and Scheduling for AI Work... Wei Chen & Mengxuan Li
- Match score: 0.926
- Transcript file: N/A
- Transcript chars: 0
- Keywords: gpu, management, dynamic, pooling, sharing, scheduling, workloads

### Resumo baseado na transcript

_Transcript indisponível; enriquecimento baseado apenas em metadados._

### Excerpt da transcript

_Sem transcript disponível._
