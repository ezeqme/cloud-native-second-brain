---
type: kubecon-session
event: "KubeCon + CloudNativeCon China 2025 - Hong Kong, China"
event_id: kccncchn2025
year: 2025
region: "China"
city: "Hong Kong"
country: "China"
event_date: "2025"
track: "Maintainer Track"
themes: ["AI ML", "Networking", "Community Governance"]
speakers: ["Jintao Zhang", "Kong Inc."]
sched_url: https://kccncchn2025.sched.com/event/1x5hW/the-next-steps-for-ingress-nginx-and-the-ingate-project-jintao-zhang-kong-inc
youtube_search_url: https://www.youtube.com/results?search_query=The+Next+Steps+for+Ingress-NGINX+and+the+Ingate+Project+CNCF+KubeCon+2025
slides: []
status: parsed
---

# The Next Steps for Ingress-NGINX and the Ingate Project - Jintao Zhang, Kong Inc.

## Identificação

- Edição: KubeCon + CloudNativeCon China 2025 - Hong Kong, China
- Trilha oficial: [[Maintainer Track]]
- Temas: [[AI ML]], [[Networking]], [[Community Governance]]
- País/cidade: China / Hong Kong
- Speakers: Jintao Zhang, Kong Inc.
- Schedule: https://kccncchn2025.sched.com/event/1x5hW/the-next-steps-for-ingress-nginx-and-the-ingate-project-jintao-zhang-kong-inc
- Busca YouTube: https://www.youtube.com/results?search_query=The+Next+Steps+for+Ingress-NGINX+and+the+Ingate+Project+CNCF+KubeCon+2025

## Resumo

Sessão da KubeCon sobre The Next Steps for Ingress-NGINX and the Ingate Project.

## Descrição oficial

_Descrição oficial não encontrada._

## Links

- Schedule oficial: https://kccncchn2025.sched.com/event/1x5hW/the-next-steps-for-ingress-nginx-and-the-ingate-project-jintao-zhang-kong-inc
- YouTube search: https://www.youtube.com/results?search_query=The+Next+Steps+for+Ingress-NGINX+and+the+Ingate+Project+CNCF+KubeCon+2025
## YouTube enrichment

- YouTube: https://www.youtube.com/watch?v=ZjXCSLpMHwo
- YouTube title: The Next Steps for Ingress-NGINX and the Ingate Project - Jintao Zhang, Kong Inc.
- Match score: 0.913
- Transcript file: N/A
- Transcript chars: 0
- Keywords: ingress-nginx, ingate

### Resumo baseado na transcript

_Transcript indisponível; enriquecimento baseado apenas em metadados._

### Excerpt da transcript

_Sem transcript disponível._
