---
type: kubecon-session
event: "KubeCon + CloudNativeCon China 2025 - Hong Kong, China"
event_id: kccncchn2025
year: 2025
region: "China"
city: "Hong Kong"
country: "China"
event_date: "2025"
track: "Keynote Sessions"
themes: ["AI ML", "SRE Reliability"]
speakers: ["Vivian Hu", "Product Manager", "Second State", "Lv Yi", "CTO", "5miles"]
sched_url: https://kccncchn2025.sched.com/event/1x5hf/keynote-how-we-save-900-per-day-with-self-hosted-ai-building-scalable-local-llm-infrastructure-vivian-hu-product-manager-second-state-lv-yi-cto-5miles
youtube_search_url: https://www.youtube.com/results?search_query=Keynote%3A+How+We+Save+%24900+per+Day+with+Self-Hosted+AI%3A+Building+Scalable+Local+LLM+Infrastructure+CNCF+KubeCon+2025
slides: []
status: parsed
---

# Keynote: How We Save $900 per Day with Self-Hosted AI: Building Scalable Local LLM Infrastructure - Vivian Hu, Product Manager, Second State & Lv Yi, CTO, 5miles

## Identificação

- Edição: KubeCon + CloudNativeCon China 2025 - Hong Kong, China
- Trilha oficial: [[Keynote Sessions]]
- Temas: [[AI ML]], [[SRE Reliability]]
- País/cidade: China / Hong Kong
- Speakers: Vivian Hu, Product Manager, Second State, Lv Yi, CTO, 5miles
- Schedule: https://kccncchn2025.sched.com/event/1x5hf/keynote-how-we-save-900-per-day-with-self-hosted-ai-building-scalable-local-llm-infrastructure-vivian-hu-product-manager-second-state-lv-yi-cto-5miles
- Busca YouTube: https://www.youtube.com/results?search_query=Keynote%3A+How+We+Save+%24900+per+Day+with+Self-Hosted+AI%3A+Building+Scalable+Local+LLM+Infrastructure+CNCF+KubeCon+2025

## Resumo

Sessão da KubeCon sobre Keynote: How We Save $900 per Day with Self-Hosted AI: Building Scalable Local LLM Infrastructure.

## Descrição oficial

_Descrição oficial não encontrada._

## Links

- Schedule oficial: https://kccncchn2025.sched.com/event/1x5hf/keynote-how-we-save-900-per-day-with-self-hosted-ai-building-scalable-local-llm-infrastructure-vivian-hu-product-manager-second-state-lv-yi-cto-5miles
- YouTube search: https://www.youtube.com/results?search_query=Keynote%3A+How+We+Save+%24900+per+Day+with+Self-Hosted+AI%3A+Building+Scalable+Local+LLM+Infrastructure+CNCF+KubeCon+2025
## YouTube enrichment

- YouTube: https://www.youtube.com/watch?v=RyITa7SMnJ0
- YouTube title: Keynote: How We Save $900 per Day with Self-Hosted AI: Building Scalable Local... Vivian Hu & Lv Yi
- Match score: 0.934
- Transcript file: N/A
- Transcript chars: 0
- Keywords: keynote, self-hosted, scalable, llm, infrastructure

### Resumo baseado na transcript

_Transcript indisponível; enriquecimento baseado apenas em metadados._

### Excerpt da transcript

_Sem transcript disponível._
